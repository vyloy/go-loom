...
func getKeygenCmd() *cobra.Command {
	...
	keygenCmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			fmt.Fprintf(os.Stderr, "written private key file '%s'\n", privFile)
			return nil
		},
	}
...
func main() {
	...
	saveStateCmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			fmt.Fprintf(os.Stdout, "set value %d for user '%s' successfully!\n", value, user)
			return nil
		},
	}
	...
	getStateCmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			fmt.Fprintf(os.Stdout, "get value %d from user '%s'\n", msgData.Value, user)
			return nil
		},
	}
